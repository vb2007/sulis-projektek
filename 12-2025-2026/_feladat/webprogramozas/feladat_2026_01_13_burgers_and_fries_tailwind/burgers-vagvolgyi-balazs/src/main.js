...
function createGroup({title = null, description = null, items = []})
{
  ...
  for(const item of items)
  {
    ...
    priceEl.textContent = item.price.toLocaleString("de-DE", { style: "currency", currency: "EUR" })
...
foodsEl.append(...Object.keys(foodList).map(group => createGroup(foodList[group])))
